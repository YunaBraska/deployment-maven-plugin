...
MVN_OPTIONS="${MVN_OPTIONS} -Dproject.build.sourceEncoding=${ENCODING} -Dproject.encoding=${ENCODING} -Dproject.reporting.outputEncoding=${ENCODING} -Dmaven.compiler.source=${JAVA_VERSION} -Dmaven.compiler.target=${JAVA_VERSION} -DuseSystemClassLoader=false -Dsurefire.useSystemClassLoader=false -Djdk.net.URLClassPath.disableClassPathURLCheck=true"
