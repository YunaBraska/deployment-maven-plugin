...
MVN_OPTIONS="${MVN_OPTIONS} -Dproject.build.sourceEncoding=${ENCODING} -Dproject.encoding=${ENCODING} -Dproject.reporting.outputEncoding=${ENCODING} -Dmaven.compiler.source=${JAVA_VERSION} -Dmaven.compiler.target=${JAVA_VERSION} -DuseSystemClassLoader=false -Djdk.net.URLClassPath.disableClassPathURLCheck=true -DargLine=-Xmx1024m -DargLine=-XX:MaxPermSize=256m -Dmaven.surefire.debug --debug"
